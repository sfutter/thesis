...
```{r}
df9 = na.omit(df)                                                                   # remove the NAs since only care about rows which have all observations. 
...
```{r}
# names(df15)
df15 = df15 %>% select(Date, Time, MMYYYY:TimeHoursNum,                  # date Time variables
...
outfilePath = file.path("~/Dropbox","NU","THESIS","DATASETS","GROOMED")
write.csv(df15, file.path(outfilePath,'greenwich_eltham_imputed_school_hours_take2.csv'))
```
