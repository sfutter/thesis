...
df                    = df %>% select( NOX, MMYYYY, Year, Month, Week, Day, TimeHoursNum, BP:WSPD, lag.1.NO:lag.5.WSPD, X5amTo8amSlope.NO2.IMP:min.6.to.8.AM.WSPD.IMP )
...
response        = c('NOX')    # NO, NOX, SO2, PM10, PM2.5  Done: NO2 (0.5, 5, 10)
results         = c()
...
dfExport$nodes = c(10) #'ceiling(input var count *0.5)')   #,10,5)
dfExport$response = c('NOX') #, 'NO', 'NO')
dfExport$X = c(9) # increase +1 each time
dfExport = dfExport %>% select(X, chosenVar.final, mae.test.latest.final.y, mae.train.latest.final.y, mae.xval.latest.final.y, nodes, response)
dfExport
