...
nodeCount = c(2) #ceiling(colcount/2),5,10) #search on
response        = c('NO2') 
...
# NEED TO LOOP THROUGH TO GET THE 
# TAKE 2 AT GETTING THE SAVED MODEL MAE VALUES ALONG WITH THEIR VALUES
```{r}
...
for (i in 17:17){
  model_path <- paste0("~/Dropbox/NU/THESIS_MAC/thesis/MODELS/NO2.dl.hidden.layers.2.colcount.2.j.loop.",i,"/colcount.3.j.",i,"variable.count.2.NO2.dl")
  saved_model <- h2o.loadModel(model_path)    # "/Users/stevenfutter/Dropbox/NU/THESIS/R CODE/~/Dropbox/NU/THESIS/MODELS/no2.dl"
...
### DELETE THIS SECTION ONCE FIGURED OUT HOW TO GET AT MIN MAE XVAL MODEL!!!
```{r}
library(dplyr)
library(lubridate)
codePath = file.path("~/Dropbox","NU","THESIS_MAC","thesis","R CODE")
...
createDataFrameNO2 = function(df,predictor){
  dfNew = df %>% select(NO2, Year, Time, lag.1.NO2, lag.2.NO2) 
  return(dfNew)
}
...
colcount    = dim(df.train.hex)[2]
hiddencount = 2  # need to run the neural network with half as many hidden nodes as input nodes
...
INSERT CHART HERE SHOWING HOW MAE DECREASES AS WE ADD NODES!
...
df = hiddenNodeRes %>% filter(response == 'NO2') %>% select(response, nodes, mae.train.latest.final, mae.test.latest.final, mae.xval.latest.final)
df = arrange(df, nodes)
df
p <- plot_ly(df, x = ~nodes, y=~mae.xval.latest.final, name = 'Cross-validation MAE', type='scatter', mode='lines+markers') %>%      
...
#  add_trace(y = ~euLimit, name = 'EU Limit', line = list(color = 'rgb(205, 12, 24)', width = 4, dash = 'dash')) %>%
  layout(title = "Cross-Validation MAE as Function of Number of Hidden Nodes", 
  xaxis = list(title  = "Number of Hidden Nodes"),
  yaxis = list (title = "Cross-validation MAE (NO2)"))
p
```
