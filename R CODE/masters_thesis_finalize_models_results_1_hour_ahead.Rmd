...
df.test.mae          # 4.92347 NO2 (w.out min/max) -> 4.980155 NO2 (w min/max) -> 4.178306 latest 10 node model!
...
# overall accuracy
overall_accuracy = (table(results$predictOver40, results$actualOver40)[1,1] + table(results$predictOver40, results$actualOver40)[2,2]) / 
                   (table(results$predictOver40, results$actualOver40)[1,1] + table(results$predictOver40, results$actualOver40)[1,2] +
                    table(results$predictOver40, results$actualOver40)[2,1] + table(results$predictOver40, results$actualOver40)[2,2]  )
paste0('Overall accuractely predict NO2: ',round(overall_accuracy,3))



```

# TEST DATA: MAE BY HOUR OF DAY
...
df.Xam.mae.output = c()
for (hour in 9:16){
 df.Xam     = df %>% filter(TimeHoursNum==hour)
 df.Xam.mae = mean(abs(df.Xam$predict-df.Xam$NO2))
 df.Xam.mae.output = append(df.Xam.mae.output, df.Xam.mae)
}

df.Xam.mae.output
mean(df.Xam.mae.output)
...
df.vali.mae          # 5.847267 NO2  -> 4.809545 NO2 on final model 10 node
...
# VALI DATA: MAE BY HOUR OF DAY

```{r}
df

# CALC the MAE here
# VALI: evaluate the quality of the prediction using the vali data set:
df.vali.mae = mean(abs(df$predict-df$NO2))
df.vali.mae          # 4.779481
...
euLimit = rep(40,8)  # 8 hours at 40 mewg/m3
...
# before i continue lets plot ALL the validation actual vs predicted
...
data = df %>% filter(df$TimeHoursNum >=9)
data

# calculate a new vector here for average hourly NO2
...
calculateMonthlyHourlyAverage = function(df, predictor) {
  ...
  meanHourlyConcentrationByMonthHourDf = df %>% select_('TimeHoursNum', predictor)  %>% group_by_('TimeHoursNum') %>% summarise_each(funs(mean))
  meanHourlyConcentrationByMonthHourDfColName = paste0('Mean.Hourly.Concentration.By.MMYYYY.',predictor)                                                    
  colnames(meanHourlyConcentrationByMonthHourDf) = c('TimeHoursNum',meanHourlyConcentrationByMonthHourDfColName)
  
  dfNew = inner_join(df, meanHourlyConcentrationByMonthHourDf, by = c("TimeHoursNum" = "TimeHoursNum"))
  return(dfNew)
}

# in validation data we have dates in feb and march - so the hourly average is over these time-periods, ONLY
dataNew = calculateMonthlyHourlyAverage(data, "NO2")
dataNew$eu.limit = rep(40, dim(dataNew)[1])

p <- plot_ly(dataNew, x = ~Time, y=~NO2, name = 'Actual NO2', type='scatter', mode='lines+markers') %>%      
  add_trace(y = ~predict, name = 'Predicted NO2',mode = 'lines+markers') %>%
  add_trace(y = ~Mean.Hourly.Concentration.By.MMYYYY.NO2, name = 'Mean Hourly NO2 (Feb/Mar 2014)',mode = 'lines+markers') %>%
  add_trace(y = ~eu.limit, name = 'EU Limit', line = list(color = 'rgb(205, 12, 24)', width = 4, dash = 'dash')) %>%
  layout(margin = list(b = 160), xaxis = list(tickangle = 45), title = paste0("Actual vs Predicted Hourly NO2 Greenwich Eltham"), 
  xaxis = list(title  = "Hour of Day"),
  yaxis = list (title = "Hourly NO2 Concentration (µg/m3)"))
p
...
# date 1: annotate graph when correctly or incorrectly predict over 40
data = resultsOver40AllDateObs %>% filter(resultsOver40AllDateObs$Date == unique(Over40AllObs$Date)[9])

good.predict = data %>% filter(actualOver40 == 'Actual Above 40' & predictOver40=='Predict Above 40')
good.predict$text = 'Accurate Prediction'
bad.predict = data %>% filter(actualOver40 == 'Actual Above 40' & predictOver40=='Predict Below 40')
bad.predict$text = 'Inaccurate Prediction'
euLimit = rep(40,11)  # 11 hours
...
p <- plot_ly(data, x = ~Time, y=~NO2, name = 'Actual NO2', type='scatter', mode='lines+markers') %>%      
  add_trace(y = ~predict, name = 'Predicted NO2',mode = 'lines+markers') %>%
  add_trace(y = ~euLimit, name = 'EU Limit', line = list(color = 'rgb(205, 12, 24)', width = 4, dash = 'dash')) %>%
  layout(annotations=a, title = paste0("Actual vs Predicted Hourly NO2 Greenwich Eltham: ",unique(Over40AllObs$Date)[9]), 
  xaxis = list(title  = "Hour of Day"),
  yaxis = list (title = "Hourly NO2 Concentration (µg/m3)"))
p
```
