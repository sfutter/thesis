...
# TEST SET
# 1. ONE-HOUR AHEAD for 8 hours in day
```{r}
...
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_test_5_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_test_6_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_test_7_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_test_8_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
df
...
df.test.mae          # 4.92347 NO2 (w.out min/max) -> 4.980155 NO2 (w min/max) -> 4.178306 latest 10 node model! (1 hr ahead) to 4.991352 for latest 10 node (8 hr ahead)
...
<!-- # TEST DATA: MAE BY HOUR OF DAY (8 hour ahead) -->

<!-- ```{r} -->
<!-- df -->

<!-- (x <- strptime(df$TimeKey1, format="%d/%m/%Y %H:%M"))          # Strip time from date -->
...
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_vali_5_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_vali_6_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_vali_7_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
dfNew      = read.csv(file.path(infilePath,"greenwich_eltham_NO2_ann_vali_8_hr_ahead_final.csv"),na.strings=c("NA"," "))
df = rbind(df, dfNew)
df
...
df.vali.mae          # 5.847267 NO2  -> 4.809545 NO2 on final model 10 node   -> 7.724349 NO2 on final 10 node 8hr ahead
